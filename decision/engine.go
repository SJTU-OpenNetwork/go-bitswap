...
var OriginIpfsNodes = []string {
    "12D3KooWEBKQAdjyqa4iMp8Lu8NF9tMQSWZoninNNPhbGYJ1xvcH",
    "QmZt8jsim548Y5UFN24GL9nX9x3eSS8QFMsbSRNMBAqKBb",
    "QmRHLRg5vihUgakbk7JnQFswWu7D92awdZnKiQRi1DmJhE",
    "QmYBXdc56TrPqKWhAYJZneLpVeG4qMaV8Be6yox3fiqBYd",
    "QmYL5AAcaGA2undBnRqWRTmndkL1YV3v7tML8DbakC8sTD",
    "QmcwtfsFoJALLQwJWmsh5SmothbrniohPcW2PuggSVKurT",
    "QmYCYQMhyDJV4BU9fRr5xBzFDEccnukuViUT7GJLngP7fj",
}

func IsOldIpfs(id string) bool{
    for _, n := range OriginIpfsNodes {
        if id == n {
            return true
        }
    }
	...
	for _, entry := range entries {
		...
		if ok {
			...
			if msgSize+blockSize > maxMessageSize {
				...
				if(taskLength+int(*e.wct) > sendTicketThreshold && !isOldIpfs){	//We do not have enough network resources. Send tickets instead of send the block
					//Create ticket
	...
	for _, l := range e.ledgerMap {
        ...
        for _, ticket := range newTickets {
			...
			if entry, ok := l.WantListContains(k); ok {
                ...
                tmpticket.SetTimeStamp(ticket.TimeStamp()-getCurrentTimestamp()+e.getTimeDuration() + 500) //add arbitrary value for this is not a good ticket
				activeTickets = append(activeTickets, tmpticket)
			}
		}
...
func (e *Engine) addBlocks(ctx context.Context, ks []cid.Cid) {
	...
	for _, l := range e.ledgerMap {
		l.lk.Lock()
        isOldIpfs := IsOldIpfs(l.Partner.String())
	    activeTickets := make([]tickets.Ticket, 0)
		for _, k := range ks {
			if entry, ok := l.WantListContains(k); ok {
                if e.ticketStore.IsSending(l.Partner, k) {
	...
	for _, ack := range acks {
        //e.findOrCreate(ack.Receiver()) //why this causes dead lock????????????????????
//        l.lk.Lock()
