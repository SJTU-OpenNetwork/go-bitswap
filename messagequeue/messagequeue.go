...
func (mq *MessageQueue) AddTicketMessage(tickets []tickets.Ticket) {
	mq.addTickets(tickets)
...
// AddTickets adds new tickets to an outgoing message.
func (mq *MessageQueue) AddTicketAckMessage(acks []tickets.TicketAck) {
	mq.addTicketAcks(acks)
...
func (mq *MessageQueue) addTickets(tickets []tickets.Ticket) {
	mq.nextMessageLk.Lock()
	defer mq.nextMessageLk.Unlock()
	// if we have no message held allocate a new one
	if mq.nextMessage == nil {
...
func (mq *MessageQueue) addTicketAcks(acks []tickets.TicketAck) {
	mq.nextMessageLk.Lock()
	defer mq.nextMessageLk.Unlock()
	// if we have no message held allocate a new one
	if mq.nextMessage == nil {
